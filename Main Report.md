...
| Test Case ID | Description | Pre-condition | Input Data | Test Steps | Expected Result | Actual Result | Status | Defect ID | Tested By | Date Tested |
| ------------ | ----------- | ------------- | ---------- | ---------- | --------------- | ------------- | ------ | --------- | --------- | ----------- |
| FR-12-DT-01 | Truy cập API Admin bằng token hợp lệ và vai trò admin | Tồn tại tài khoản có quyền Admin hợp lệ trong hệ thống. | Phương thức HTTP: GET<br>Đường dẫn API: /api/admin/users<br>Authorization: Bearer {{admin_token}}<br>Role trong token: admin<br>Trạng thái token: hợp lệ, còn hạn | 1. Mở Postman hoặc Swagger UI.<br>2. Gọi POST /api/login với credentials hợp lệ của tài khoản admin để lấy token thật.<br>3. Chọn phương thức GET cho đường dẫn `/api/admin/users`.<br>4. Thêm tiêu đề `Authorization` với giá trị `Bearer {{admin_token}}`.<br>5. Gửi yêu cầu. | Hệ thống cho phép truy cập và trả về phản hồi thành công theo nghiệp vụ của API. | Hệ thống cho phép truy cập và trả về phản hồi thành công theo nghiệp vụ của API. | Passed | | Hoang | 27/06/2026 |
| FR-12-DT-02 | Truy cập API Admin khi thiếu token JWT | Không yêu cầu. | Phương thức HTTP: GET<br>Đường dẫn API: /api/admin/users<br>Authorization: rỗng<br>Role trong token: không có<br>Trạng thái token: không tồn tại | 1. Mở Postman hoặc Swagger UI.<br>2. Chọn phương thức GET cho đường dẫn `/api/admin/users`.<br>3. Không nhập tiêu đề `Authorization`.<br>4. Gửi yêu cầu. | Hệ thống từ chối truy cập vì thiếu token JWT hợp lệ. |  Hệ thống từ chối truy cập vì thiếu token JWT hợp lệ. | Passed | | Hoang | 27/06/2026 |
| FR-12-DT-03 | Truy cập API Admin bằng token hợp lệ nhưng vai trò user | Tồn tại tài khoản quyền User thông thường hợp lệ trong hệ thống. | Phương thức HTTP: GET<br>Đường dẫn API: /api/admin/users<br>Authorization: Bearer {{user_token}}<br>Role trong token: user<br>Trạng thái token: hợp lệ, còn hạn | 1. Mở Postman hoặc Swagger UI.<br>2. Gọi POST /api/login với credentials hợp lệ của tài khoản USER thường để lấy token thật.<br>3. Chọn phương thức GET cho đường dẫn `/api/admin/users`.<br>4. Thêm tiêu đề `Authorization` với giá trị `Bearer {{user_token}}`.<br>5. Gửi yêu cầu. | Hệ thống từ chối truy cập vì vai trò trong token không phải `admin`. | Hệ thống cho phép truy cập và trả về phản hồi thành công theo nghiệp vụ của API. | Failed | BUG-FR12-01 | Hoang | 27/06/2026 |
| FR-12-DT-04 | Gọi API Admin bằng token có chữ ký sai | Tồn tại tài khoản có quyền Admin hợp lệ trong hệ thống. | Phương thức HTTP: GET<br>Đường dẫn API: /api/admin/users<br>Authorization: Bearer {{invalid_signature_token}}<br>Role trong token: admin<br>Trạng thái token: chữ ký không hợp lệ | 1. Mở Postman hoặc Swagger UI.<br>2. Gọi POST /api/login với credentials hợp lệ của tài khoản admin để lấy token thật, sau đó cố tình sửa đổi vài ký tự ở phần cuối của token (phần signature).<br>3. Chọn phương thức GET cho đường dẫn `/api/admin/users`.<br>4. Thêm tiêu đề `Authorization` với giá trị `Bearer {{invalid_signature_token}}`.<br>5. Gửi yêu cầu. | Hệ thống từ chối truy cập vì token JWT không hợp lệ do chữ ký sai. | Hệ thống từ chối truy cập vì token JWT không hợp lệ do chữ ký sai. | Passed | | Hoang | 27/06/2026 |
## Boundary Value Analysis
### Giải thích
